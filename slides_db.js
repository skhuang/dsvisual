...
const SLIDES_DB = {
  ...
  "search-kmp": {
    ...
    "slides": [
      {
        "heading": { "zh": "KMP 字串比對演算法", "en": "KMP String Matching" },
        "blocks": [
          { "type": "paragraph", "text": {
      ...
      },
      {
        ...
        "blocks": [
          ...
          { "type": "bullets", "items": [
              { "zh": "LPS 陣列在 $O(m)$ 時間內預建完成。", "en": "The LPS array is built in $O(m)$ time." },
              { "zh": "文字指標 `i` 永不後退，每步至少前進一格。", "en": "The text index `i` never moves backward; it advances by at least one step each time." },
              { "zh": "總比對次數 $\\le 2n$，整體複雜度 $O(n+m)$。", "en": "Total comparisons $\\le 2n$; overall complexity $O(n+m)$." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "steps", "items": [
              { "zh": "**預處理**：對模式字串執行 `computeLPS`，建立 LPS 陣列。", "en": "**Preprocess**: run `computeLPS` on the pattern to build the LPS array." },
              { "zh": "**匹配推進**：當 `text[i] == pat[j]` 時同時令 `i++` 與 `j++`；若 `j == m` 則記錄一次成功匹配，並令 `j = lps[j-1]`。", "en": "**Match advance**: when `text[i] == pat[j]` increment both `i` and `j`; when `j == m` record a match and set `j = lps[j-1]`." },
              { "zh": "**不匹配處理**：若 `j > 0`，令 `j = lps[j-1]`（不移動 `i`）；若 `j == 0`，令 `i++`。", "en": "**Mismatch**: if `j > 0`, set `j = lps[j-1]` (keep `i` fixed); if `j == 0`, increment `i`." }
          ] },
      ...
      },
      {
        "heading": { "zh": "示意圖", "en": "Layout" },
        "blocks": [
          { "type": "svg", "svg": "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 420 90\" width=\"420\"><g font-family=\"monospace\" font-size=\"13\"><text x=\"0\" y=\"20\">Pattern:  A  B  A  B  C  A  B  A  B</text><text x=\"0\" y=\"45\">Index:    0  1  2  3  4  5  6  7  8</text><text x=\"0\" y=\"70\">LPS:      0  0  1  2  0  1  2  3  4</text></g></svg>" },
          { "type": "note", "text": {
      ...
      },
      {
        ...
        "blocks": [
          { "type": "table",
            ...
            "rows": [
              [ { "zh": "建立 LPS", "en": "Build LPS" }, { "zh": "$O(m)$", "en": "$O(m)$" }, { "zh": "$O(m)$", "en": "$O(m)$" } ],
              [ { "zh": "搜尋", "en": "Search" }, { "zh": "$O(n)$", "en": "$O(n)$" }, { "zh": "$O(1)$", "en": "$O(1)$" } ],
              [ { "zh": "總計", "en": "Total" }, { "zh": "$O(n+m)$", "en": "$O(n+m)$" }, { "zh": "$O(m)$", "en": "$O(m)$" } ]
            ] },
          { "type": "math", "tex": "T_{\\text{KMP}} = O(n + m)", "caption": {
      ...
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "優點：保證線性時間，文字指標絕不後退。", "en": "Pro: guaranteed linear time; the text pointer never backtracks." },
              { "zh": "優點：LPS 陣列建構簡單，只需 $O(m)$ 時間與空間。", "en": "Pro: the LPS array is straightforward to precompute in $O(m)$ time and space." },
              { "zh": "缺點：常數因子高於 Boyer-Moore，在一般文字上實際速度較慢。", "en": "Con: constant factor higher than Boyer-Moore; slower in practice on typical text." },
              { "zh": "缺點：未利用字母表特性，無法像 BM 那樣大幅跳移。", "en": "Con: does not exploit the alphabet; cannot make large skips like Boyer-Moore." },
              { "zh": "適用：最壞情況線性保證至關重要的場景（例如重複性高的字串）。", "en": "Use when worst-case linearity matters, e.g. highly repetitive strings." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "LPS（failure function）是 KMP 的核心預處理資料。", "en": "The LPS (failure function) array is the key precomputed data structure in KMP." },
              { "zh": "文字指標永不後退；總比對次數 $\\le 2n$。", "en": "The text pointer never retreats; total comparisons $\\le 2n$." },
              { "zh": "時間 $O(n+m)$，空間 $O(m)$。", "en": "Time $O(n+m)$, space $O(m)$." },
              { "zh": "與 BM、RK 的比較見 search-strcompare。", "en": "For a comparison with BM and RK, see search-strcompare." }
          ] }
        ]
      }
    ]
  ...
  "search-bm": {
    ...
    "slides": [
      {
        "heading": { "zh": "Boyer-Moore 字串比對演算法", "en": "Boyer-Moore String Matching" },
        "blocks": [
          { "type": "paragraph", "text": {
      ...
      },
      {
        ...
        "blocks": [
          ...
          { "type": "bullets", "items": [
              { "zh": "從右至左比對使得每次不匹配都能跳過多個字元。", "en": "Right-to-left scanning enables large jumps on every mismatch." },
              { "zh": "預處理時間 $O(m + \\sigma)$，$\\sigma$ 為字母表大小。", "en": "Preprocessing time is $O(m + \\sigma)$ where $\\sigma$ is the alphabet size." },
              { "zh": "最佳情況下每次跳移 $m$ 個字元，總比對次數 $O(n/m)$。", "en": "Best case skips $m$ characters at a time, giving $O(n/m)$ total comparisons." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "steps", "items": [
              { "zh": "**預處理**：呼叫 `buildBadChar` 建立壞字元表；呼叫 `buildGoodSuffix` 建立好後綴移位表。", "en": "**Preprocess**: call `buildBadChar` to build the bad-character table and `buildGoodSuffix` to build the good-suffix shift table." },
              { "zh": "**右至左比對**：從 `j = m-1` 往左比對；遇到不匹配即停止。", "en": "**Right-to-left compare**: match from `j = m-1` downward; stop on the first mismatch." },
              { "zh": "**移位**：若完全匹配則以 `shift[0]` 移位；否則取 `max(goodSuffix, badChar)` 移位。", "en": "**Shift**: on a full match apply `shift[0]`; on mismatch shift by `max(good-suffix, bad-character)`." }
          ] },
      ...
      },
      {
        ...
        "blocks": [
          { "type": "table",
            "headers": [ { "zh": "階段", "en": "Phase" }, { "zh": "時間", "en": "Time" }, { "zh": "空間", "en": "Space" } ],
            "rows": [
              [ { "zh": "預處理", "en": "Preprocessing" }, { "zh": "$O(m + \\sigma)$", "en": "$O(m + \\sigma)$" }, { "zh": "$O(m + \\sigma)$", "en": "$O(m + \\sigma)$" } ],
              [ { "zh": "搜尋（最佳）", "en": "Search (best)" }, { "zh": "$O(n/m)$", "en": "$O(n/m)$" }, { "zh": "—", "en": "—" } ],
              [ { "zh": "搜尋（最差）", "en": "Search (worst)" }, { "zh": "$O(nm)$", "en": "$O(nm)$" }, { "zh": "—", "en": "—" } ]
            ] },
          { "type": "math", "tex": "T_{\\text{BM,best}} = O(n/m)", "caption": {
      ...
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "優點：實際效能為次線性，在大型字母表與長文字上是最快的通用比對器。", "en": "Pro: sublinear in practice; the fastest general-purpose matcher on large alphabets and long texts." },
              { "zh": "優點：右至左掃描讓壞字元跳移通常比 KMP 大。", "en": "Pro: right-to-left scanning often yields much larger skips than KMP." },
              { "zh": "缺點：不加 Galil 規則則最壞情況為 $O(nm)$。", "en": "Con: worst case is $O(nm)$ without the Galil rule." },
              { "zh": "缺點：需要壞字元與好後綴兩張預處理表，實作較複雜。", "en": "Con: two preprocessing tables (bad-character and good-suffix) make the implementation more complex." },
              { "zh": "適用：大型字母表（如自然語言文字）、長模式字串。", "en": "Use for large alphabets (e.g. natural language text) and long patterns." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "從右至左掃描；壞字元 + 好後綴雙啟發。", "en": "Right-to-left scan; bad-character and good-suffix heuristics." },
              { "zh": "平均次線性；最差 $O(nm)$。", "en": "Average sublinear; worst case $O(nm)$." },
              { "zh": "大型字母表與長文字的首選演算法。", "en": "The go-to algorithm for large alphabets and long texts." },
              { "zh": "與 KMP、RK 的比較見 search-strcompare。", "en": "For a comparison with KMP and RK, see search-strcompare." }
          ] }
        ]
      }
    ]
  ...
  "search-rk": {
    ...
    "slides": [
      {
        "heading": { "zh": "Rabin-Karp 字串比對演算法", "en": "Rabin-Karp String Matching" },
        "blocks": [
          { "type": "paragraph", "text": {
      ...
      },
      {
        ...
        "blocks": [
          ...
          { "type": "bullets", "items": [
              { "zh": "雜湊命中不代表一定匹配（偽陽性），需逐字元驗證。", "en": "A hash hit may be a false positive (collision) and requires character-by-character verification." },
              { "zh": "選取良好的模數可使碰撞率極低，偽陽性概率 $O(1/MOD)$。", "en": "A good modulus keeps collision rate low; false-positive probability is $O(1/MOD)$." },
              { "zh": "天然延伸至多模式搜尋：同時比對多個模式的雜湊值。", "en": "Extends naturally to multi-pattern search by comparing against multiple pattern hashes simultaneously." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "steps", "items": [
              { "zh": "**初始化**：計算模式字串的雜湊值 `patHash` 與第一個視窗的雜湊值 `winHash`；預計算高位乘法因子 `h`。", "en": "**Initialize**: compute the pattern hash `patHash` and the first window hash `winHash`; precompute the high-order multiplier `h`." },
              { "zh": "**滾動更新**：每次視窗右滑一格，以 $O(1)$ 更新 `winHash`：去除最左字元，加入最右字元。", "en": "**Rolling update**: on each slide, update `winHash` in $O(1)$: remove the leftmost character's contribution and add the new rightmost character." },
              { "zh": "**驗證**：當 `patHash == winHash` 時，逐字元比對確認是否為真正匹配。", "en": "**Verify**: when `patHash == winHash`, compare characters one by one to confirm a genuine match." }
          ] },
      ...
      },
      {
        ...
        "blocks": [
          { "type": "table",
            "headers": [ { "zh": "階段", "en": "Phase" }, { "zh": "時間", "en": "Time" }, { "zh": "空間", "en": "Space" } ],
            "rows": [
              [ { "zh": "預處理", "en": "Preprocessing" }, { "zh": "$O(m)$", "en": "$O(m)$" }, { "zh": "$O(1)$", "en": "$O(1)$" } ],
              [ { "zh": "搜尋（平均）", "en": "Search (average)" }, { "zh": "$O(n+m)$", "en": "$O(n+m)$" }, { "zh": "—", "en": "—" } ],
              [ { "zh": "搜尋（最差）", "en": "Search (worst)" }, { "zh": "$O(nm)$", "en": "$O(nm)$" }, { "zh": "$O(1)$", "en": "$O(1)$" } ]
            ] },
          { "type": "math", "tex": "T_{\\text{RK,avg}} = O(n + m)", "caption": {
      ...
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "優點：額外空間 $O(1)$，實作簡單，天然支援多模式搜尋。", "en": "Pro: $O(1)$ extra space, simple to implement, extends naturally to multi-pattern search." },
              { "zh": "優點：平均 $O(n+m)$，在低碰撞率下效能接近 KMP。", "en": "Pro: average $O(n+m)$; performs close to KMP under low collision rates." },
              { "zh": "缺點：對抗性輸入下碰撞率高，退化為 $O(nm)$。", "en": "Con: adversarial inputs can force $O(nm)$ through high collision rates." },
              { "zh": "缺點：每次雜湊命中都需額外的驗證步驟。", "en": "Con: every hash hit requires an extra character-by-character verification step." },
              { "zh": "適用：多模式搜尋、抄襲偵測與文件指紋比對。", "en": "Use for multi-pattern search, plagiarism detection, and document fingerprinting." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "滾動雜湊讓每次視窗滑動只需 $O(1)$ 更新。", "en": "Rolling hash allows $O(1)$ update per window slide." },
              { "zh": "雜湊命中時才驗證；平均 $O(n+m)$，空間 $O(1)$。", "en": "Verify only on hash hit; average $O(n+m)$, space $O(1)$." },
              { "zh": "最適合多模式搜尋與指紋比對應用場景。", "en": "Best suited for multi-pattern search and fingerprinting applications." },
              { "zh": "與 KMP、BM 的比較見 search-strcompare。", "en": "For a comparison with KMP and BM, see search-strcompare." }
          ] }
        ]
      }
    ]
  ...
  "search-strcompare": {
    ...
    "slides": [
      {
        "heading": { "zh": "字串比對演算法比較", "en": "String Matching Compared" },
        "blocks": [
          { "type": "paragraph", "text": {
      ...
      },
      {
        ...
        "blocks": [
          ...
          { "type": "bullets", "items": [
              { "zh": "KMP：LPS（failure function）—— 保證線性時間，文字指標永不後退。", "en": "KMP: LPS / failure function — guaranteed linear time, text pointer never retreats." },
              { "zh": "BM：壞字元 + 好後綴啟發 —— 實際次線性，大型字母表最快。", "en": "BM: bad-character + good-suffix heuristics — sublinear in practice, fastest on large alphabets." },
              { "zh": "RK：滾動雜湊 —— 平均線性，空間 $O(1)$，天然支援多模式。", "en": "RK: rolling hash — average linear, $O(1)$ space, naturally multi-pattern." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "steps", "items": [
              { "zh": "**初始化**：visualizer 以相同文字 `ABABDABACDABABCABAB` 與模式 `ABABCABAB` 同步啟動三個比對窗格。", "en": "**Initialize**: the visualizer starts all three panes simultaneously on the shared text `ABABDABACDABABCABAB` and pattern `ABABCABAB`." },
              { "zh": "**逐步推進**：每按一次「下一步」，三個演算法各前進一個邏輯步驟，各自的比對計數器分別累計。", "en": "**Step together**: each press of \"Next\" advances all three algorithms by one logical step; their individual comparison counters accumulate independently." },
              { "zh": "**完成比較**：某個演算法先找到所有匹配後停止；visualizer 顯示各自的比對次數供比較。", "en": "**Compare results**: when an algorithm finishes finding all matches it stops; the visualizer displays each algorithm's comparison count for direct comparison." }
          ] },
      ...
      },
      {
        ...
        "blocks": [
          { "type": "table",
            "headers": [ { "zh": "演算法", "en": "Algorithm" }, { "zh": "時間（最差）", "en": "Time (worst)" }, { "zh": "時間（平均／最佳）", "en": "Time (avg/best)" }, { "zh": "空間", "en": "Space" } ],
            "rows": [
              [ { "zh": "KMP", "en": "KMP" }, { "zh": "$O(n+m)$", "en": "$O(n+m)$" }, { "zh": "$O(n+m)$", "en": "$O(n+m)$" }, { "zh": "$O(m)$", "en": "$O(m)$" } ],
              [ { "zh": "Boyer-Moore", "en": "Boyer-Moore" }, { "zh": "$O(nm)$", "en": "$O(nm)$" }, { "zh": "$O(n/m)$（最佳）", "en": "$O(n/m)$ (best)" }, { "zh": "$O(m + \\sigma)$", "en": "$O(m + \\sigma)$" } ],
              [ { "zh": "Rabin-Karp", "en": "Rabin-Karp" }, { "zh": "$O(nm)$", "en": "$O(nm)$" }, { "zh": "$O(n+m)$（平均）", "en": "$O(n+m)$ (avg)" }, { "zh": "$O(1)$", "en": "$O(1)$" } ]
            ] },
          { "type": "math", "tex": "O(n + m) \\text{ — shared baseline}", "caption": {
      ...
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "選 KMP：需要最壞情況線性保證，或字串重複性高（二進位 DNA 序列）。", "en": "Choose KMP: when worst-case linear time is required, e.g. highly repetitive binary or DNA strings." },
              { "zh": "選 BM：追求在大型字母表與長文字上的最快實際速度（自然語言、程式碼搜尋）。", "en": "Choose BM: for raw speed on large alphabets and long texts (natural language, code search)." },
              { "zh": "選 RK：多模式搜尋、記憶體受限、抄襲偵測或文件指紋比對場景。", "en": "Choose RK: for multi-pattern search, memory-constrained environments, plagiarism detection, or document fingerprinting." }
          ] }
        ]
      },
      {
        ...
        "blocks": [
          { "type": "bullets", "items": [
              { "zh": "同一問題，三種取捨：KMP 保證線性、BM 實際最快、RK 最省空間。", "en": "Same problem, three trade-offs: KMP guaranteed linear, BM fastest in practice, RK most space-thrifty." },
              { "zh": "KMP 適合重複性高的字串；BM 適合大型字母表；RK 適合多模式比對。", "en": "KMP suits repetitive strings; BM suits large alphabets; RK suits multi-pattern matching." },
              { "zh": "三者皆為精確比對演算法；$O(n+m)$ 只是比較的參考基線，三者的最差複雜度其實不同（見複雜度表）。", "en": "All three are exact-matching algorithms; $O(n+m)$ is only the reference baseline for comparison — their worst-case complexities actually differ (see the complexity table)." },
              { "zh": "詳細說明見 search-kmp、search-bm、search-rk。", "en": "For detailed coverage see search-kmp, search-bm, and search-rk." }
          ] }
        ]
      }
    ]
