...
// Tree Node for visualization logic
class VizTreeNode {
    constructor(val, x, y, dx) {
        this.val = val;
        this.left = null;
        this.right = null;
        this.x = x;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    modeRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            ...
            // reset state for simplicity
            stackData = [];
            qArr = new Array(5).fill(null);
            qFront = 0; qRear = -1; qCount = 0;
            edges = [];
            bstRoot = null;
    ...
    btnStdAdd.addEventListener('click', () => {
        ...
        if(currentMode.includes('stack')) {
            if(currentMode === 'stack-array' && stackData.length >= MAX_SIZE) {
                return showStatus('Stack Overflow!', '#f87171');
            }
            stackData.push(val);
            showStatus("Pushed " + val, '#60a5fa');
            renderStack('push');
        } else if (currentMode === 'queue') {
            if (qCount >= MAX_SIZE) {
                return showStatus('Queue Overflow!', '#f87171');
            }
            qRear = (qRear + 1) % MAX_SIZE;
            qArr[qRear] = val;
            qCount++;
            showStatus("Enqueued " + val, '#60a5fa');
            renderQueue('enqueue');
        }
    ...
    btnStdRemove.addEventListener('click', () => {
        if(currentMode.includes('stack')) {
            ...
            showStatus("Popped " + popped, '#ec4899');
            renderStack('pop');
        } else if (currentMode === 'queue') {
            if(qCount === 0) return showStatus('Queue Underflow!', '#f87171');
            const deq = qArr[qFront];
            qArr[qFront] = null;
            qFront = (qFront + 1) % MAX_SIZE;
            qCount--;
            showStatus("Dequeued " + deq, '#ec4899');
            renderQueue('dequeue');
        }
    });
    ...
    btnGraphAdd.addEventListener('click', () => {
        ...
        if(isNaN(u) || isNaN(v) || u<0 || u>4 || v<0 || v>4) {
            return showStatus('Invalid Nodes (0-4)', '#f87171');
        }
        if(u === v) return showStatus('No self loops', '#f87171');
        
        if(edges.some(e => (e[0]===u && e[1]===v) || (e[0]===v && e[1]===u))) {
            return showStatus('Edge already exists', '#f87171');
        }

        edges.push([u, v]);
        showStatus("Added edge: " + u + " - " + v, '#60a5fa');
        renderGraph();
    });
    ...
    btnTreeAdd.addEventListener('click', () => {
        ...
        function insertBST(node, v, x, y, dx) {
            if(!node) {
                showStatus("Inserted " + v, '#60a5fa');
                return new VizTreeNode(v, x, y, dx);
            }
            if(v < node.val) {
                node.left = insertBST(node.left, v, node.x - node.dx, node.y + 60, node.dx * 0.6);
            } else if (v > node.val) {
                node.right = insertBST(node.right, v, node.x + node.dx, node.y + 60, node.dx * 0.6);
            } else {
                showStatus(v + " already exists", '#f87171');
            }
            return node;
        }

        // root starts at center top: x=200, y=30, dx=100 (half width span)
        bstRoot = insertBST(bstRoot, val, 200, 30, 90);
        treeVal.value = Math.floor(Math.random() * 100);
        renderTree();
    });
    ...
    function updateLayout() {
        ...
        linkedListContainer.classList.add('hidden');
