...
| ③ | OJ 題 + **測資自動生成** | md + `oj:` + code library 參考解 + 每題產生器 | `dsjudge/problems/<id>/`(meta.yaml + statement + 參考解 + 自動產的 `tests/NN.in/.out`) | dsjudge 完整 runner、`window.open_at`、`type:lab/exam` |
...
**真正要新寫的程式**只有:md directive 解析器、ipynb 生成器、SVG helper library、linter、**OJ 測資產生器框架**。其餘是「撰寫內容 + 接既有管線」。

### 6.1 OJ 測資自動生成 (生成器 ③ 細節)

採標準 OJ 模式「**輸入產生器 + 參考解當 oracle**」:

```
每題 gen 規格 ──► 輸入產生器 ──► tests/NN.in
                                     │
                  code library 參考解 ◄┘ (跑過)
                                     │
                                     ▼
                                tests/NN.out  (= 參考解輸出,即期望答案)
```

- **輸入產生器**:每題一支 `gen.py`(或 `gen.cpp`),依約束(範圍、邊界、隨機種子)吐多組 `.in`。種子固定 → 可重現。
- **oracle = 參考解**:直接用 code library 的參考解跑 `.in` 得 `.out`,期望答案與「正確程式」永遠一致(避免人工答案出錯)。
- **必含案例類別**:邊界(空、單元素、最大規模)、隨機、對拍(可選第二參考解交叉驗證)。
- **人工只需提供** gen 規格(約束 + 案例類別),其餘自動;測資仍存 dsjudge 學生不可達 trusted dir。
- gen 規格的承載方式(OJ directive 參數 vs 每題獨立 `gen.*` 檔)於 **Phase 2 細化**。
...
| **2 OJ** | OJ 題生成 + 測資產生器框架;對 chap03 產 `lab` 題(`open_at` 延後公開) | 自動產的測資能在 dsjudge runner 判題;邊界/隨機案例齊備;open_at 前學生不可見 |
...
- **測資自動生成**(見 §6.1)——人工只提供 gen 規格,不手寫測資;不做的是「全自動猜測約束」(約束需人工指定)。
...
- OJ gen 規格承載方式:directive 參數 vs 每題 `gen.*` 檔(Phase 2 定)。

## 12. 已解決 (本次更新)

- **ds2026 納入 git(private)** — 權威來源原本未版控,已建立 private repo `skhuang/ds2026`,使講義變更可追溯、可觸發 CI linter。產出(`.html`)、教科書 PDF(版權)、`.backup`、`node_modules` 以 `.gitignore` 排除。
- **OJ 測資改為自動生成**(§6.1)——推翻原 YAGNI「測資人工撰寫」。
