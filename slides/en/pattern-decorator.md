...
Decorator is a Structural design pattern that dynamically attaches additional responsibilities to an object. By sharing the same interface between decorator and decorated object, responsibilities can be chained in any combination — more flexible than inheritance.

---

## Core Concept

The `Coffee` interface is the shared contract. `SimpleCoffee` is the base implementation. `CoffeeDecorator` holds a `shared_ptr<Coffee>` and implements the same interface, delegating calls to the wrapped object while layering its own behaviour.

- Component interface (`Coffee`): declares `getDescription()` and `getCost()`.
- Concrete Component (`SimpleCoffee`): the original undecorated object.
- Decorator base (`CoffeeDecorator`): implements `Coffee` and holds a pointer to the wrapped object.
- Concrete Decorators (`MilkDecorator`, `SugarDecorator`, `WhippedCreamDecorator`): each layers its own description and cost.

---

## Operation Flow

1. Create a base `SimpleCoffee` object (description: "Simple Coffee", cost: USD 2.00).
2. Wrap with `MilkDecorator`: `getDescription()` returns "Simple Coffee + Milk", cost adds USD 0.50.
3. Wrap with `SugarDecorator`: description appends "+ Sugar", cost adds another USD 0.25.
4. Any number of decorators can be stacked; each call accumulates results through chained delegation.
...
> Decorator and Component share the same interface; the Decorator base holds a Component pointer forming a self-referential structure. Concrete decorators extend the Decorator base, adding behaviour before delegating to the inner object.
...
| Intent | Dynamically attach responsibilities — more flexible than inheritance |
| Chain depth $n$ | Call time $O(n)$, space $O(n)$ |
| Principle | Composition over Inheritance |

$$\text{cost}(n) = \text{cost}_0 + \sum_{i=1}^{n} \Delta_i$$

The total cost of a decorator chain equals the base component cost plus the sum of all increments added by the decorators.
...
- Pro: dynamic runtime composition — more flexible than subclassing, avoiding class explosion.
- Pro: follows Single Responsibility Principle — each Decorator handles exactly one added responsibility.
- Pro: follows Open/Closed Principle — add a new Decorator without modifying existing classes.
- Con: many small Decorator classes complicate the structure and make call-chain tracing during debugging harder.
- Use for I/O stream wrapping (e.g. `BufferedReader`), GUI component decoration, billing/logging/auth as cross-cutting concerns.

---

## Summary

- Decorator is a Structural pattern: Decorator and Component share an interface; composition enables dynamic responsibility attachment.
- A chain of $n$ Decorators incurs $O(n)$ call time.
- Core principle: Composition over Inheritance — composition is more flexible and preserves encapsulation.
- Classic use: C++ `std::istream`/`std::ostream` family wrappers all embody the Decorator idea.
