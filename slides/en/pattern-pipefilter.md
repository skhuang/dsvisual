...
Data flows through a chain of independent filters connected by pipes; each filter transforms its input and passes the result on to the next.

---

## Core Concept

Each filter has a single responsibility and a uniform input/output interface, so filters compose and reorder freely. The pipeline is simply an ordered list of filters.

- The uniform input → output interface allows filters to be chained in any order.
- Filters are independent of each other and individually reusable.
- The pipeline is just an ordered list of filters, enabling flexible composition.

---

## Operation Flow

1. Build the pipeline by adding filters in the desired order.
2. Feed the input data into the first filter of the pipeline.
3. Each filter transforms the data and forwards it to the next, producing the final output.
...
> The visualizer shows the filter chain horizontally: Input → Trim → Upper → Exclaim → Output, with connectors representing data flow (pipes).
...
| Aspect | Notes |
| --- | --- |
| Composability & reuse | Filters compose freely, can be reordered, and reused across different pipelines |
| Per-stage testability | Each filter can be tested in isolation with direct input and output verification |
| Cost | Per-stage data copying adds overhead; not ideal for tight feedback loops |
...
- Pro: filters compose and reorder freely for maximum flexibility.
- Pro: each filter can be unit-tested in complete isolation.
- Con: copying data between stages adds performance overhead.
- Con: awkward to apply for non-linear data flows.
- Use for stream and batch data transformation (compilers, data pipelines, text processing).

---

## Summary

- Pipe-and-Filter decomposes data processing into a chain of single-responsibility, independent filters.
- The uniform interface enables free composition of filters into highly reusable pipelines.
- Widely applied in compiler front-ends, ETL pipelines, and text-processing scenarios.
