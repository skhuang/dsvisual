...
Aho-Corasick builds a trie of multiple patterns, then computes failure links by BFS, so a single text scan finds every occurrence of every pattern.

---

## Core Concept

The trie supplies goto edges; a failure link points to the node for the longest proper suffix of the current string, so a mismatch never rewinds the text.

- Failure links are computed by BFS, shallow-to-deep.
- Output links are merged along the failure chain, so no match is missed.
- The text pointer never moves backward during the scan.

---

## Operation Flow

1. Insert every pattern into the trie.
2. BFS-compute each node's failure link and merge outputs.
3. Scan the text, advancing along goto or jumping back along failure links, reporting a match whenever an output node is reached.
...
> The visualizer has two stepped phases: first build the failure links, then scan the text `ushers`.
...
| Aspect | Complexity |
| --- | --- |
| build time | $O(\sum |P_i|)$ |
| scan | $O(|text| + \#matches)$ |
| space | $O(\sum |P_i| \cdot \sigma)$ |

$$T_{\text{scan}} = O(|text| + \#matches)$$

The text pointer never rewinds, so the scan is linear.
...
- Pro: handles many patterns in one scan, and the text never rewinds.
- Con: must build the automaton, and space grows with total pattern length and alphabet.
- Use for multi-keyword filtering, intrusion detection, and dictionary matching.

---

## Summary

- Trie goto edges plus BFS-built failure links.
- One linear scan finds every pattern.
- It is KMP generalized to multiple patterns.
