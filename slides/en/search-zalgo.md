...
The Z-algorithm builds a string's Z-array in linear time: $Z[i]$ is the longest length, starting at $i$, that matches a prefix of the string; it enables $O(n+m)$ string matching.

---

## Core Concept

Maintain an $[l, r]$ window (the rightmost segment known to match a prefix) so indices inside it can reuse already-computed values.

- When $i$ is inside $[l,r]$, start from the mirror value $Z[i-l]$.
- When $i$ is outside the window, compare characters from zero.
- Update $[l,r]$ after comparing.

---

## Operation Flow

1. Concatenate `pattern`, a separator character, and `text` into one string.
2. Compute the Z-array of the concatenation.
3. Every index where $Z[i]$ equals the pattern length is a match.
...
- Pro: simple concept, linear time, and no failure-function table.
- Con: needs the extra concatenated string and $O(n+m)$ space.
- Use for single-pattern matching, string periodicity, and prefix analysis.

---

## Summary

- The Z-array gives the length of the prefix match.
- The $[l,r]$ window achieves linear time.
- Concatenating pattern, separator, and text does the matching.
