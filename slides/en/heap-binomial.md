...
> $B_0$ is a single node; $B_1$ links two $B_0$ trees; $B_2$ links two $B_1$ trees. Each $B_k$ has exactly $2^k$ nodes and root degree $k$.
...
| peek | $O(\log N)$ | $O(1)$ |
| extractTop (worst) | $O(\log N)$ | $O(1)$ |
| merge (worst) | $O(\log N)$ | $O(1)$ |
| decreaseKey (worst) | $O(\log N)$ | $O(1)$ |
| Total Space | — | $O(N)$ |

$$T_{\text{merge}}(N) = O(\log N)$$

Merging two size-$N$ binomial heaps processes at most $2\lfloor\log_2 N\rfloor+2$ trees, giving $O(\log N)$.
...
- Pro: $O(\log N)$ merge — much better than Binary Heap's $O((N+M)\log(N+M))$.
- Pro: $O(1)$ amortized insert; efficient for sequential insertions.
- Con: pointer-based; worse cache performance than array-based Binary Heap.
- Con: peek requires a linear scan of the root list — $O(\log N)$, not $O(1)$.
- Use when efficient merges are critical: event-driven simulators, external-merge Prim MST.

---

## Summary

- $N$ elements → at most $\lfloor\log_2 N\rfloor+1$ trees; at most one tree per degree.
- Merge $O(\log N)$; insert $O(1)$ amortized; extractTop $O(\log N)$ worst-case.
- Structure mirrors binary addition: linking equal-degree trees is analogous to carrying.
