...
A Ternary Search Tree (TST) is a hybrid of BST and Trie: each node stores one character and has three child pointers (left / eq / right), combining prefix search capability with low memory overhead.

---

## Core Concept

Each node stores one character `data` with three child pointers: left (current char is smaller), eq (current char matches — advance to next character), right (current char is larger).

- Insert: compare character by character; equal → take eq to next char; less → go left; greater → go right.
- Search time $O(\log N + L)$: horizontal BST traversal ~$O(\log N)$, vertical character matching $O(L)$.
- Space: only 3 pointers per node — far superior to Trie's 26 pointers per node.
- `isEndOfWord` flag is set on the node corresponding to the last character.

---

## Operation Flow

1. If node is `nullptr`: allocate a new node storing the current character.
2. If `word[index] < node->data`: recurse into `left` child; index unchanged.
3. If `word[index] > node->data`: recurse into `right` child; index unchanged.
4. If equal: if last character set `isEndOfWord`; otherwise recurse into `eq` child and increment index.
...
> After inserting "CAR" then "CAT": C-A share the eq chain; at the third character R vs T — since T > R, the T node becomes the right child of R.
...
| --- | --- | --- |
| insert string | $O(\log N + L)$ | $O(L)$ |
| search string | $O(\log N + L)$ | $O(1)$ |
| Total Space | — | $O(N \cdot L)$ |

$$T_{\text{search}} = O(\log N + L)$$

$\log N$ from horizontal BST comparisons; $L$ from vertical character matching. Only 3 pointers/node vs. 26 in Trie.
...
- Pro: 3 pointers per node — far more memory-efficient than Trie's 26 pointers.
- Pro: supports prefix search — better than plain BST for string queries.
- Con: horizontal BST comparisons add $O(\log N)$ overhead compared to Trie's $O(L)$.
- Use for spell checking, word suggestion, and approximate string matching in memory-sensitive scenarios.

---

## Summary

- Three pointers (left/eq/right) fuse BST horizontal comparison with Trie vertical character matching.
- Search $O(\log N + L)$ with only 3 pointers/node — an efficient Trie alternative.
- Best for memory-constrained prefix search, e.g. embedded spell checkers.
