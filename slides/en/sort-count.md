...
Counting Sort avoids element comparisons: it counts the frequency of each key, then uses cumulative counts to place each element directly at its correct output position. Both time and space are $O(n+k)$, where $k$ is the value range.

---

## Core Concept

Three steps: ① build count array `count[k]` for frequencies; ② prefix-sum `count` so `count[i]` is the number of elements $\leq i$; ③ scan original array from back to front, place each element at `output[count[val]-1]` and decrement (ensuring stable).

- Backward scan ensures stable: among equal keys, later elements are placed later.
- Restriction: only for integers (or data mappable to integer keys); wastes space when $k$ is very large.
- Most efficient when $k \ll n$, e.g. sorting ages, scores, or other bounded integer domains.

---

## Operation Flow

1. Find max and min; range = max - min + 1.
2. Create `count[range]` (zero-init), count frequencies: `count[arr[i]-min]++`.
3. Prefix sum: `count[i] += count[i-1]`.
4. Back-to-front: `output[count[arr[i]-min]-1] = arr[i]; count[arr[i]-min]--`.
...
> After prefix sum, count[2]=3 means there are 3 elements $\leq 2$, so the last occurrence of 2 goes to output[2] (0-indexed). Back-to-front placement ensures stability.
...
$n$ = number of elements, $k$ = value range (max-min+1). When $k = O(n)$ the overall cost is $O(n)$, breaking the comparison-sort $O(n \log n)$ lower bound.
...
- Pro: stable, $O(n+k)$ linear time — beats the comparison-sort lower bound.
- Pro: used as a subroutine in Radix Sort (LSD digit-by-digit sort).
- Con: only for bounded integer keys; space waste grows with $k$.
- Con: NOT in-place, needs $O(n+k)$ extra memory.
- Use for large volumes of bounded integer keys like ages, scores, or ASCII characters.

---

## Summary

- Non-comparison sort: count → prefix sum → backward placement.
- stable; $O(n+k)$ always; $O(n+k)$ auxiliary space.
- Optimal linear sort for bounded integer domains; also the core of Radix Sort.
