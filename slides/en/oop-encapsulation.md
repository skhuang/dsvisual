...
Encapsulation bundles data and the methods that operate on that data into a single class. Access specifiers (`private`, `protected`, `public`) hide implementation details while exposing a controlled `public` interface.

---

## Core Concept

The `BankAccount` example demonstrates typical encapsulation: `balance` is `private`; external code can only interact via `deposit()`, `withdraw()`, and `getBalance()`, preserving all invariants.

- `public`: part of the class contract; accessible from anywhere.
- `protected`: accessible by the class and its derived classes; typically used for overridable helper methods.
- `private`: accessible only within the class; protects data from direct external modification.
- `const` member function: cannot modify object state, providing a read-only access guarantee.
- `mutex` + `lock_guard`: protect shared state for thread safety inside `deposit`/`withdraw`.

---

## Operation Flow

1. Caller invokes `account.deposit(200)`; enters the `public` method, which acquires a `lock_guard`.
2. Inside, the method directly accesses the `private` member `balance`, completing the logic before releasing the lock.
3. `withdraw` calls the `protected` method `canWithdraw()` and the `private` method `log()`, neither of which is directly callable externally.
4. External code can only read the balance through `getBalance()` (a `public const` method), with no direct modification path.
...
- Pro: invariant protection — `private` members can only be modified through controlled methods, preventing invalid states.
- Pro: interface stability — internals can be freely refactored without affecting callers as long as the `public` interface is unchanged.
- Pro: thread safety — locks can be centralised at `public` method entries; external code cannot bypass them.
- Con: over-encapsulation can produce verbose getter/setter boilerplate.
- Use whenever internal state must be protected or thread safety is needed, e.g. accounts, caches, connection pools.

---

## Summary

- Encapsulation = data hiding + controlled access: access specifiers define the boundaries.
- `private` guards invariants; `protected` supports inheritance extension; `public` defines the contract.
- Access control is enforced at compile time with zero runtime overhead.
- Good encapsulation is the foundation of maintainable, testable, and thread-safe code.
