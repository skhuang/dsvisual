...
Singleton is a Creational design pattern that ensures a class has exactly one instance throughout the program's lifetime and provides a global access point to it.

---

## Core Concept

A `private` constructor prevents external instantiation; a `static` member pointer holds the sole instance; `static getInstance()` implements lazy initialization, protected by a `mutex` for thread safety.

- Private constructor: blocks external `new` or direct declarations.
- Copy constructor / assignment operator deleted: prevents a second copy.
- Static member pointer (`m_instance`): holds the unique instance, initially `nullptr`.
- `getInstance()` with `lock_guard`: ensures thread-safe first-time creation.

---

## Operation Flow

1. First call to `Singleton::getInstance()`: enters critical section, `m_instance == nullptr` holds, creates the unique instance.
2. Subsequent calls: enter the critical section but `m_instance != nullptr`, so the existing pointer is returned.
3. All callers hold the same pointer; modifications to the instance are globally visible.
4. The destructor is called at program termination, cleaning up the single instance.
...
> `m_instance` and `m_mutex` are static members in the static data segment, shared across all callers. `getInstance()` is the only legal creation/access point.
...
| Participants | Singleton class itself |
| Intent | Ensure one instance and provide global access |
| Initialization | Lazy (on first call) |
| Thread Safety | Requires mutex protection |

$$|\{\text{instances of } C\}| = 1$$

Core Singleton invariant: the set of instances of class $C$ always has exactly one element.
...
- Pro: guarantees a single global instance, avoiding resource conflicts or inconsistent state from multiple objects.
- Pro: lazy initialization saves resources; the object is created only when first needed.
- Con: global state makes unit testing harder; watch for state leakage between tests.
- Con: hides dependencies, violating the Explicit Dependency Principle.
- Use for Logger, Configuration manager, Database connection pool — scenarios requiring a single shared instance.

---

## Summary

- Singleton is a Creational pattern: private constructor + static getInstance() ensures a single instance.
- Lazy initialization + mutex protection is the standard approach in multi-threaded environments.
- Copy constructor and assignment operator must be explicitly deleted to prevent accidental copies.
- Use global state cautiously; modern C++ favours the Meyers Singleton (static local variable) over raw pointer.
