...
A LIFO stack built from dynamically allocated `Node` objects in a linked list — no fixed capacity, grows and shrinks on demand.

---

## Core Concept

Each node holds a data field and a pointer to the next node. `topNode` always points to the head of the list (the stack top).

- push: allocate a new node, set its `next` to the old top, then update `topNode`.
- pop: read `topNode->data`, advance `topNode` to the next node, `delete` the old node.
- No Stack Overflow; limited only by available heap memory.

---

## Operation Flow

1. Allocate a new node and initialise its `data`.
2. Set the new node's `next` to the current `topNode`.
3. Update `topNode` to point to the new node — push complete.
4. To pop: save the top value, advance `topNode`, then free the old node.
...
> Each node lives at a scattered heap address; `next` pointers chain them together. `topNode` always points to the most recently pushed node.
...
Each push allocates one node and updates two pointers — independent of stack size.
...
- Pro: dynamically sized — no capacity limit needs to be set in advance.
- Pro: no wasted pre-allocated space.
- Con: each node carries a pointer overhead; memory is non-contiguous (less cache-friendly).
- Use when element count is unpredictable, e.g. DFS traversal, expression evaluation.

---

## Summary

- LIFO: implemented by keeping `topNode` at the list head.
- Both push and pop are $O(1)$ with no hard capacity limit.
- Compared to array implementation, trades pointer overhead for dynamic sizing.
