...
Shaker Sort (Cocktail Sort) is a bidirectional Bubble Sort: a forward pass bubbles the largest element to the right, then a backward pass sinks the smallest element to the left, alternating to speed up migration of small elements toward the start.

---

## Core Concept

Maintain `left` and `right` boundaries. After each forward pass, `right--`; after each backward pass, `left++`. If either pass has no swaps, exit early.

- Solves Bubble Sort's "turtle problem": small elements near the end take many forward passes to reach the head.
- stable: comparison `arr[i-1] > arr[i]` uses strict greater-than, equal elements are not swapped.
...
1. Initialise `left=0, right=n-1, swapped=false`.
2. Forward scan `i` from `left` to `right-1`: swap if `arr[i]>arr[i+1]`; then `right--`.
3. If no swap occurred in this direction, break.
4. Backward scan `i` from `right` to `left+1`: swap if `arr[i-1]>arr[i]`; then `left++`.
...
> Green marks the left boundary (smallest settled), red marks the right boundary (largest settled). Both boundaries shrink by one after each pair of passes, excluding already-sorted elements.
...
Bidirectional scanning halves the number of passes for nearly-sorted data with small elements near the end, but the asymptotic complexity remains $O(n^2)$.
...
- Pro: stable, in-place; faster than Bubble Sort for arrays with small elements near the end.
- Pro: bidirectional scanning reduces the number of passes in many cases.
- Con: average and worst case remain $O(n^2)$; cannot compete with Quick/Merge Sort for large datasets.
- Use for educational purposes or small nearly-sorted datasets where occasional small elements are far from their correct position.

---

## Summary

- Bidirectional bubble: forward moves the maximum right, backward moves the minimum left.
- stable, in-place; best $O(n)$, average/worst $O(n^2)$.
- Mitigates Bubble Sort's turtle problem, but asymptotic complexity is unchanged; suited for teaching and small nearly-sorted inputs.
