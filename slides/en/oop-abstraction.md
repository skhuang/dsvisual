...
Abstraction defines what (the interface) while hiding how (the implementation); in C++, an abstract class whose members are all pure virtual functions is an interface.

---

## Core Concept

A class with at least one pure virtual function (`virtual ... = 0;`) is abstract and cannot be instantiated directly. A derived class must `override` every pure virtual function to become a concrete, instantiable class.

- A pure virtual function `= 0` declares a contract without providing an implementation.
- An abstract class cannot be instantiated (`Shape s;` fails to compile), but a base pointer/reference can point to a derived object.
- A C++ interface is nominal: the derived class must explicitly inherit `: public Base` for the compiler to accept it.

---

## Operation Flow

1. Declare an abstract base class; write the contract methods as pure virtual `= 0`, and provide a virtual destructor.
2. Each concrete derived class overrides every pure virtual function.
3. Use objects through a base pointer/reference; calls dispatch at runtime to the actual type.
...
> The visualizer draws the abstract `Shape` as a dashed box; the two concrete subclasses `Circle` and `Rectangle` each implement `area()`.
...
| Aspect | Cost |
| --- | --- |
| Abstract class itself | Zero extra runtime cost |
| Virtual call via base pointer | One vtable pointer indirection |
| Each polymorphic object | One extra vptr pointer |

$$T_{\text{virtual call}} = T_{\text{direct call}} + O(1)$$

A virtual call costs just one constant-time table indirection more than a direct call.
...
- Pro: decouples interface from implementation — callers depend only on the contract.
- Pro: forces derived classes to implement every contract method, or they stay non-instantiable.
- Con: a virtual call has one indirection; and a C++ interface is nominal — explicit inheritance is required.
- Versus Go: a Go interface is structural (duck-typed) — a type satisfies it automatically by having the methods, with no inheritance; both are forms of subtype polymorphism.

---

## Summary

- An abstract class is a contract with pure virtual functions; all-pure-virtual = a C++ interface.
- A C++ interface is nominal (explicit inheritance); a Go interface is structural.
- Abstraction completes the four pillars of OOP (encapsulation, abstraction, inheritance, polymorphism).
