...
Bucket Hashing divides the array into fixed-capacity buckets; each bucket holds multiple elements (2 slots per bucket in this implementation). Insertions go directly into the bucket when space is available; a full bucket triggers linear probing to the next adjacent bucket.

---

## Core Concept

Each `Bucket` struct contains a fixed-size slot array (`slots[2]`) and a `count`. The hash function `key % NUM_BUCKETS` determines the primary bucket; when full, the next bucket is probed linearly.

- Each bucket has a fixed capacity of 2 (this implementation), providing a small collision buffer.
- Primary bucket not full: insert directly at `slots[count++]`, $O(1)$.
- Primary bucket full: linear probe `idx = (idx+1) % NUM_BUCKETS` until a bucket with space is found.
- Insertion fails if all buckets are fully saturated.

---

## Operation Flow

1. Compute primary bucket index: `idx = key % NUM_BUCKETS`.
2. If `table[idx].count < 2`, write to `slots[count++]` and return.
3. Otherwise set `idx = (idx+1) % NUM_BUCKETS` and repeat step 2 until a bucket with space is found.
4. If the probe wraps back to the start without finding space, print "Catastrophic Failure".
...
> 10 and 14 both hash to Bucket[2] (`key%4==2`), filling its two slots. 22 also hashes to Bucket[2] but finds it full, so linear probing places it in Bucket[3].
...
| insert | $O(1)$ | $O(B)$ | $O(1)$ |
| search | $O(1)$ | $O(B)$ | $O(1)$ |
| Total Space | — | — | $O(B \cdot K)$ |

$$\text{Capacity} = B \times K$$

$B$ = number of buckets, $K$ = slots per bucket ($K=2$ in this implementation). Average $O(1)$; worst case probes all $B$ buckets.
...
- Pro: multiple slots per bucket provide a small collision buffer, reducing probe frequency.
- Pro: data stored in a contiguous array structure — cache-friendly.
- Pro: fixed capacity design makes memory usage precisely predictable.
- Con: fixed bucket capacity; overflow still requires adjacent-bucket probing, degrading at high load.
- Con: fixed slot count may waste space (a bucket with 1 element leaves 1 slot unused).
- Use for bounded datasets needing simple, predictable memory footprint, e.g. embedded systems or hardware-mapped address schemes.

---

## Summary

- Each bucket has $K$ fixed slots; $O(1)$ insertion when the primary bucket has space.
- A full primary bucket triggers linear probing to adjacent buckets; worst case $O(B)$.
- Combines cache-friendliness with a collision buffer; total capacity $B \times K$ is fixed.
