...
> Bucket 3's linked list has 43 at the head (most recently inserted) followed by 33 (both satisfy `key % 5 == 3`). Because insert prepends at the head, the last-inserted key 43 appears first.
...
- Load factor $\alpha = n/m$ determines average chain length; separate chaining allows $\alpha > 1$, but keeping $\alpha$ bounded by a small constant ensures $O(1)$ average operations.
