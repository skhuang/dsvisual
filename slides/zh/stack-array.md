...
堆疊是一種 LIFO(後進先出)的線性結構,如同一疊盤子:最後放上的最先被取下。

---

## 核心概念

陣列實作用一塊固定大小的連續記憶體,搭配一個整數索引 `top` 指向頂端元素。

- `top` 初始為 -1,代表空堆疊。
- `push` 先遞增 `top` 再寫入;`pop` 先讀取再遞減 `top`。
- 容量固定,推入超過上限會發生 Stack Overflow。

---

## 運作流程

1. 檢查 `top` 是否已達容量上限,若是則拒絕推入。
2. 遞增 `top`,將新值寫入 `arr[top]`。
3. 彈出時讀取 `arr[top]`,再將 `top` 遞減。
...
> 藍色為已使用的格子,白色為閒置空間;`top` 永遠指向最後一個有效元素。
...
- 優點:記憶體連續、無指標額外開銷,快取友善。
- 缺點:容量固定,需事先決定大小。
- 適用:容量上限已知的場景,如函式呼叫堆疊、括號配對。
