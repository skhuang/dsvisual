...
跳躍列表是一種以多層連結串列建構的有序映射;隨機層級使搜尋、插入、刪除的期望複雜度為 $O(\log n)$,是平衡樹的一種簡單替代方案。

---

## 核心概念

第 0 層保存每一個節點;較高的層級是「快速車道」,只串接部分節點;搜尋時從最高層往下逐層推進。
...
1. 從表頭的最高層開始。
2. 當下一個節點的鍵值小於目標時往右移,否則往下降一層。
3. 到達第 0 層後,檢查下一個節點是否等於目標。
...
> visualizer 以 4 層堆疊呈現;Insert 擲硬幣決定節點高度,Search 逐步動畫呈現往下推進的路徑。
...
- 優點:比平衡樹更易實作,且支援有序走訪。
- 缺點:最差情況會退化,效能取決於亂數品質。
- 適用:需要有序映射,但想避開紅黑樹複雜度的場景。
