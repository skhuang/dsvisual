...
所有樹根形成循環雙向鏈結串列(root list),`best` 指向最小根。consolidate 在 extractTop 時才整理結構。decrease-key 透過 cut 將節點移至根串列,再以 cascading-cut 確保父節點最多損失一個子節點。

- insert:建立新節點加入根串列,若鍵值更小則更新 `best` — 純 $O(1)$。
- merge:將兩個根串列拼接,更新 `best` — $O(1)$ 攤銷。
- consolidate:將根串列中度數相同的樹兩兩合併,使每個度數至多一棵(在 extractTop 中呼叫)。
- cascading-cut:若父節點已失去一個子節點(mark=true),則繼續向上 cut。

---

## 運作流程

1. decrease-key(x, k):令 x.key = k;若 k < parent.key,執行 cut(x)。
2. cut(x):從父節點子串列移除 x,加入根串列,清除 mark。
3. cascading-cut(parent):若 parent.mark=false 設為 true 並停止;若 true 繼續 cut(parent)。
4. extractTop:將 best 的子節點全部移至根串列,呼叫 consolidate 整理,更新 best。
...
> 根串列為循環雙向鏈結;`best` 指向最小根(key=1)。子樹可掛在任意根節點下,consolidate 前不整理。
...
攤銷分析基於勢能函數 $\Phi = t + 2m$($t$ 為根串列長度,$m$ 為有 mark 的節點數),保證 decrease-key 攤銷 $O(1)$。
...
- 延遲合併根串列:insert/merge/decrease-key 皆 $O(1)$ 攤銷。
- extractTop 觸發 consolidate:攤銷 $O(\log N)$。
- 理論最優但實作複雜;適合以 decrease-key 為核心的圖演算法。
