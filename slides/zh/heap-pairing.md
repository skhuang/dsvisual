...
paginate: true
math: katex
title: "配對堆積(Pairing Heap)"
---

## 配對堆積(Pairing Heap)

以 meld(配對連結)為核心原語的多叉樹堆積:insert 與 merge 為 $O(1)$,extractTop 執行成對合併(two-pass pairing),攤銷 $O(\log N)$,實際效能接近 Fibonacci Heap。

---

## 核心概念

每個節點儲存第一個子節點指標與下一個兄弟指標。meld 操作比較兩根的鍵值,鍵值較大者成為較小者的第一個子節點。extractTop 移除根後對其所有子節點執行 two-pass pairing:先左到右兩兩配對,再右到左合併。

- meld(a, b):令鍵值較小的根為 a;b.sibling = a.child;a.child = b;回傳 a — $O(1)$。
- insert(x):建立新節點,meld 至主堆積 — $O(1)$。
- extractTop:移除根,對子節點串列執行 mergePairs — 攤銷 $O(\log N)$。

---

## 運作流程

1. extractTop:根移除後取得其子節點串列(兄弟鏈)。
2. mergePairs — 第一遍(左至右):取出前兩個子節點,meld 為一棵樹,繼續處理剩餘串列。
3. mergePairs — 第二遍(右至左):將第一遍產生的成對樹從右到左依序 meld。
4. 最終得到一棵符合 heap 性質的樹,其根為新最小值。
...
> 根的所有子節點以兄弟鏈(sibling chain)相連,插入時只需改變頭部指標。extractTop 時對整條兄弟鏈進行兩遍配對合併。
...
| decrease-key (近似) | $O(\log N)$ | $O(1)$ |
| 空間合計 | — | $O(N)$ |

$$T_{\text{extractTop}} = O(\log N)\text{ amortized}$$

Pairing Heap 的精確攤銷界尚未完全被證明達到 Fibonacci Heap 的 $O(1)$ decrease-key;extractTop 已被證明攤銷 $O(\log N)$。
...
- 優點:實作遠比 Fibonacci Heap 簡單,實際效能接近甚至超越後者。
- 優點:insert 與 merge 均為 $O(1)$,extractTop 攤銷 $O(\log N)$。
- 缺點:decrease-key 的精確攤銷界仍是開放問題(目前已知上界為 $O(2^{2\sqrt{\log \log N}})$)。
- 缺點:指標結構,快取效能不如陣列式堆積。
- 適用:需要高效 insert/merge 的實際應用,是 Fibonacci Heap 更易實作的替代品。

---

## 小結

- 多叉樹以兄弟鏈表示;meld 為 $O(1)$ 核心操作。
- insert/merge $O(1)$;extractTop 攤銷 $O(\log N)$;實際效能優異。
- 比 Fibonacci Heap 更簡單,是業界首選的高效優先佇列實作之一。
