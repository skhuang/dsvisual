...
> 桶 3 的鏈結串列頭端為最後插入的 43,其後接 33(均滿足 `key % 5 == 3`)。insert 採頭端插入,因此後插入的 43 位於鏈首。其餘桶為 null。
...
平均鏈長等於 load factor $\alpha$;當 $\alpha$ 有界時 search 為 $O(1)$ 平均。最壞情況:所有 $N$ 個 key 雜湊至同一桶,退化為 $O(N)$。
...
- 優點:load factor 可超過 1,不需預留充裕空間。
- 優點:碰撞僅影響該桶的串列,不引發全表位移。
- 優點:刪除簡單:直接從串列摘除節點即可。
- 缺點:每個節點需額外指標空間,記憶體不連續,快取效能較差。
- 缺點:雜湊函數分佈不均時,長鏈退化為 $O(N)$。
- 適用:無法預知元素數量、需要簡單刪除、或 load factor 高的場景。

---

## 小結

- 每個桶維護一條鏈結串列;碰撞在桶內就地解決。
- 平均操作 $O(1)$,最壞 $O(N)$;空間 $O(N+M)$。
- load factor $\alpha = n/m$ 決定平均鏈長;分離鏈結法允許 $\alpha > 1$,但將 $\alpha$ 維持在小常數可確保操作平均 $O(1)$。
